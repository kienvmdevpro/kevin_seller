...
angular.module('app').controller('UploadLamidoCtrl', 
		   ['$scope', '$http', '$state','$filter', '$window', '$stateParams', 'editableThemes', 'FileUploader', 'toaster', 'Api_Path', 'Config_Status', 'Order', 'Location',
 	function($scope,   $http,   $state,  $filter,   $window,   $stateParams,    editableThemes,  FileUploader,   toaster,   Api_Path,    Config_Status,   Order,   Location) {
    /*
        Config
    */
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            ...
            if(!result.error){
               // toaster.pop('success', 'Thông báo', 'Upload Thành công!');
                ...
                $state.go('order.lamido',{id:result.id});
            }
            else{
                //toaster.pop('warning', 'Thông báo', 'Upload Thất bại!');
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_UpLoadLoiHayThuLai'));
            }
        };
