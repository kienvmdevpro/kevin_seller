...
//List Order
angular.module('app').controller('PrintNewCtrl', ['$scope', '$http', '$state', '$window', '$stateParams', 'toaster', 'Print', 'Api_Path',
        ...
        Print.Hvc($stateParams.code).then(function (result) {
            if(!result.data.error){
                ...
                angular.forEach($scope.data, function (value){
                	value.total_item_product = 0;
                	angular.forEach(value.order_items, function (item){
                		value.total_item_product = value.total_item_product + item.quantity
