...
angular.module('app').controller('ManagementCtrl',
		   ['$scope','$rootScope','$filter','$modal','$http','$stateParams','$timeout','toaster','FileUploader','Config_Status','PhpJs','Api_Path','Ticket','$state','Analytics',
 	function($scope,  $rootScope,  $filter,  $modal,  $http,  $stateParams, $timeout,   toaster,  FileUploader,  Config_Status,  PhpJs,  Api_Path, Ticket,   $state,   Analytics) {
        ...
        $scope.$watch('keyLang', function (Value, OldValue) {
        	if($rootScope.keyLang == 'en'){
        		$scope.list_status       = Config_Status.Ticket_en;
        		 $scope.data_status       = Config_Status.ticket_btn_en;
            }else if($rootScope.keyLang == 'thailand'){
            	$scope.list_status       = Config_Status.Ticket_thailand;
            	 $scope.data_status       = Config_Status.ticket_btn_thailand;
            }else{
            	$scope.list_status       = Config_Status.Ticket;
            	 $scope.data_status       = Config_Status.ticket_btn;
            }
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            if(!result.error){
            	...
            	toaster.pop('warning', tran('TICM_thongbao'), tran('TICM_uploadthatbai'));
            }
        };
        
        uploader.onErrorItem  = function(item, result, status, headers){
            //toaster.pop('error', 'Error!', "Upload file lỗi, hãy thử lại.");
            toaster.pop('error', tran('TICM_thongbao'), tran('TICM_uploadfileloihaythulai'));
        };
            ...
            if(frmstatus != ''){
                ...
                $http({
                    ...
                    if(!result.error){
                        ...
                        toaster.pop('warning', tran('TICM_thongbao'),tran('TICM_taidanhsachyeucauloi'));
                	...
                	toaster.pop('warning', tran('TICM_thongbao'), tran('Toaster_ketnoidulieuthatbai'));
                });
            }
            $scope.detail   = {};
            return;
...
angular.module('app').controller('TicketDetailCtrl', 
		    ['$scope','$modal','$http','$stateParams','$filter','$timeout','toaster','FileUploader','Config_Status','PhpJs','Api_Path','Ticket','Analytics',
    function($scope,   $modal,  $http, $stateParams,   $filter,  $timeout,  toaster,   FileUploader,   Config_Status,   PhpJs,  Api_Path,   Ticket,    Analytics) {
        ...
        $timeout(function (){
            ...
            $http({
                ...
                if(!result.error){
                	...
                	toaster.pop('warning', tran('TICM_thongbao'), tran('TICM_taicauhoidanhgiadichvu'));
                }
            }).error(function (data, status, headers, config) {
                //toaster.pop('error', 'Thông báo', 'Lỗi hệ thống!');
                toaster.pop('error', tran('TICM_thongbao'), tran('TICM_Loihethong'));
            });
                ...
                return $http({
                    ...
                    if(result.error){
                        //toaster.pop('warning', 'Thông báo', 'Tải dữ liệu lỗi !');
                    	 toaster.pop('warning',tran('TICM_thongbao'), tran('TICM_Taidulieuloi'));
                    }
                    ...
                    //toaster.pop('warning', 'Thông báo', 'Kết nối dữ liệu thất bại, vui lòng thử lại!');
                	toaster.pop('warning', tran('TICM_thongbao'), tran('TICM_Loihethong'));
                })
            }
