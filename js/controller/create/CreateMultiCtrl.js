...
angular.module('app').controller('CreateMultiCtrl', 
		   ['$scope', '$rootScope','$stateParams','$filter',  '$http', '$timeout', 'PhpJs', 'toaster','Inventory', 'Location', 'OrderStatus', 'Order', 'ConfigShipping', 'Analytics',
 	function($scope,   $rootScope,  $stateParams,  $filter,   $http,    $timeout,   PhpJs,   toaster,  Inventory,   Location,   OrderStatus,   Order,   ConfigShipping,   Analytics) {
        var timeout;
        ...
        $scope.config_fee       = {};
        $scope.list_order       = [{waiting: false, loadingLocations : false, service : '2', total_quantity: 1, checked: true, check: true }, {waiting: false, loadingLocations : false, service : '2', total_quantity: 1, checked: true, check: true }];
        ...
        $scope.create_multi = function(key){
            ...
            toaster.pop('success',tran('toaster_ss_nitifi'), tran('Toaster_KetThuc'));
        }

        var get_data = function(item){
            var data        = {'Domain'    : 'shipchung.vn'};
            
            if(!$scope.fee.Inventory.post_office || $scope.fee.Inventory.post_office == false){
                data['From']    = {
                    Stock: $scope.fee.Inventory.id
                };
            }else {
                data['From'] = {};
                data['From']['PostCode'] = $scope.fee.Inventory.id;
            }
        ...
        $scope.$watch('fee.Inventory', function (Value, OldValue){
            if (Value) {
        ...
        $scope.change   = function(item){
            ...
            if(
                ...
                $scope.fee.Inventory.id == undefined || $scope.fee.Inventory.id == 0 ||
        ...
        $scope.create   = function(item, key){
            ...
            if($scope.fee.Inventory.id == undefined || $scope.fee.Inventory.id == 0){
            ...
            if(item.to_location == undefined || item.to_location == ''
                ...
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanChuaChonTPQH'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }
            ...
            if(item.weight == undefined || item.weight == ''){
                ...
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_ChuaNhapKhoiLuong'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }

            if(item.amount == undefined || item.amount == ''){
                //toaster.pop('warning', 'Thông báo', 'Bạn chưa nhập giá sản phẩm !');
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanCHuaNhapGia'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }

            if(item.config == undefined || item.config == ''){
                //toaster.pop('warning', 'Thông báo', 'Bạn chưa chọn hình thức vận chuyển !');
            	toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanChuaConHinhThucVC'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }
            ...
            if(item.to_name == undefined || item.to_name == '' || item.to_phone == undefined || item.to_phone == ''
                ...
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanCHuaNhapThongTinNN'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }

            if(item.name == undefined || item.name == ''){
                //toaster.pop('warning', 'Thông báo', 'Bạn chưa nhập tên sản phẩm !');
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanChuaNhapTenSP'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }
            ...
            if(item.money_collect == undefined){
              //  toaster.pop('warning', 'Thông báo', 'Hãy thử lại!');
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_HayThuLai'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }
