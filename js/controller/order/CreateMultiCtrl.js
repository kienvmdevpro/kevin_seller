...
angular.module('app').controller('CreateMultiCtrl', 
		   ['$scope', '$rootScope', '$http','$filter', '$timeout', 'PhpJs', 'toaster','Inventory', 'Location', 'OrderStatus', 'Order', 'ConfigShipping', 'Analytics',
 	function($scope,   $rootScope,  $http,   $filter,   $timeout,  PhpJs,   toaster,   Inventory,   Location,   OrderStatus,   Order,   ConfigShipping,   Analytics) {
        var timeout;
        ...
        $scope.create   = function(item, key){
            ...
            if(item.courier == undefined || item.courier.courier_id == undefined || item.courier.courier_id < 1){
               // toaster.pop('warning', 'Thông báo', 'Bạn chưa chọn hãng vận chuyển !');
                toaster.pop('warning',tran('toaster_ss_nitifi'), tran('Toaster_BanChuaCHonHVC'));
                return key != undefined ? $scope.create_multi(key + 1) : true;
            }
