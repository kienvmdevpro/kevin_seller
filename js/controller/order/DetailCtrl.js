...
angular.module('app').controller('DetailCtrl', 
		   ['$scope', '$rootScope', '$http', '$filter','$state', '$stateParams', '$window', 'toaster', 'Order',
 	function($scope,  $rootScope,    $http,   $filter,   $state,    $stateParams,   $window,   toaster,    Order) {
 	  // Config
	   var tran = $filter('translate');
        ...
        $scope.list_status_order = {};
         ...
         $scope.$watch('keyLang', function (Value, OldValue) {
            	if($scope.data && $scope.data.service_id && $scope.data.service_id==1){
                    if(Value == "en"){
                        $scope.data.service_temp = $scope.data.service;
                        $scope.data.service = "Economy delivery service";
                    }else{
                        $scope.data.service = $scope.data.service_temp ? $scope.data.service_temp : $scope.data.service
                    }
                }
                if($scope.data && $scope.data.service_id && $scope.data.service_id==2){
                    if(Value == "en"){
                        $scope.data.service_temp = $scope.data.service;
                        $scope.data.service = "Express delivery service";
                    }else{
                        $scope.data.service = $scope.data.service_temp ? $scope.data.service_temp : $scope.data.service
                    }
                }
                if($scope.data && $scope.data.service_id && $scope.data.service_id==8){
                    if(Value == "en"){
                        $scope.data.service_temp = $scope.data.service;
                        $scope.data.service = "International Express";
                    }else{
                        $scope.data.service = $scope.data.service_temp ? $scope.data.service_temp : $scope.data.service
                    }
                }
                if($scope.data && $scope.data.service_id && $scope.data.service_id==9){
                    if(Value == "en"){
                        $scope.data.service_temp = $scope.data.service;
                        $scope.data.service = "International Economy";
                    }else{
                        $scope.data.service = $scope.data.service_temp ? $scope.data.service_temp : $scope.data.service
                    }
                }
