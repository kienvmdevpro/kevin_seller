...
//Verify
angular.module('app').controller('WarehouseVerifyCtrl', ['$scope', '$http', '$state', '$window', '$stateParams', 'toaster', 'Verify', 'Config_Accounting', 'Order', 'Api_Path', 'Analytics','$rootScope',
 	function($scope, $http, $state, $window, $stateParams, toaster, Verify, Config_Accounting, Order, Api_Path, Analytics,$rootScope) {
    // config
    ...
    $scope.$watch('keyLang', function (Value, OldValue) {
    	$scope.list_status 		= (Value =="en") ? $scope.list_status_en : $scope.list_status_vi;
    ...
    $scope.exportExcel = function (item){
        ...
        $scope.setPage = function(){
            ...
            $scope.list_data        = [];
            $scope.waiting          = true;
        	Verify.WarehouseVerify(time_start, time_end, search, $scope.currentPage,'').then(function (result) {
