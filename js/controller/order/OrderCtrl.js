...
angular.module('app').controller('OrderCtrl', 
		     ['$rootScope','$scope','$filter','toaster','$modal','OrderStatus','$http','$state','$window','$stateParams','Order','Inventory','Config_Status','PhpJs','$anchorScroll','$location','loginService',
    function ($rootScope,  $scope,   $filter,  toaster,  $modal,  OrderStatus,  $http,  $state, $window,   $stateParams,  Order, Inventory,   Config_Status,  PhpJs,  $anchorScroll,  $location,  loginService) {
        // config
        ...
        var checking_en = {
            1: "Receiver can view product before",
            2: "Don't buyer allow open box"
        };
        var list_cheking_vi = [
        ...
        var list_cheking_en = [
            {"id": 1, "name": "Receiver can view product before"},
            {"id": 2, "name": "Don't buyer allow open box"}
        ];

        $scope.checking = checking_vi;
        $scope.list_checking = list_cheking_vi;
        $scope.$watch('keyLang', function (Value, OldValue) {
            if(Value == 'vi'){
                ...
                $scope.checking         = checking_en;
                $scope.list_checking    = list_cheking_en;
            }
        });
...
angular.module('app').controller('OrderDetail', 
			['$scope','$rootScope','$http','$state','OrderStatus',  '$window','$stateParams','$modal','bootbox','Print','Order','$location','toaster','$filter',
    function ($scope, $rootScope,   $http,  $state,  OrderStatus,   $window,  $stateParams,  $modal,  bootbox,  Print,  Order, $location,   toaster,  $filter) {
        var tran = $filter('translate');
        ...
        if ($scope.code.length > 0) {
            Print.Multi($scope.code).then(function (result) {
                ...
                var _getListService = function(isNoiTinh){
                    $scope.list_service =  $rootScope.keyLang =='en' ? OrderStatus.list_service_en : OrderStatus.list_service;
                    if(isNoiTinh){
                    }else{
                        var list_services_temp = []
                        angular.forEach($scope.list_service, function (value){
                            if(value.group_en.toString() == "Interprovincial"){
                                list_services_temp.push(value)
                            }
                        })
                        $scope.list_service    = list_services_temp
                        
                    }
                    
                }
                if(parseInt($scope.data.from_city_id) == parseInt($scope.data.to_order_address.city_id)){
                    _getListService(true);
                }else{
                    _getListService(false);
                }
                var currentDate = new Date();
        ...
        $scope.copyOrder = function (order) {
            ...
            if(order.order_items != undefined && order.order_item != undefined){
                angular.forEach(order.order_items, function (value){
                    if(value.item != undefined){
                        ...
                        newOrder["Order"]["Description"]    = value.description;
                    }
                });
            }
