...
//List Order
angular.module('app').controller('UploadCtrl', ['$scope', '$http', '$state', '$window', '$rootScope', 'toaster', 'FileUploader', 'Api_Path', 'Analytics',
 	function($scope, $http, $state, $window, $rootScope, toaster, FileUploader, Api_Path, Analytics) {
    // upload  excel
        ...
        */
        ...
        uploader.onSuccessItem = function(item, result, status, headers){
            ...
            if(!result.error){
                ...
                $state.go('order.upload3',{id:result.id});
            }          
            else{
                toaster.pop('warning', 'Thông báo', 'Upload thất bại, vui lòng kiểm tra lại file và thử lại!');
            }
        };
